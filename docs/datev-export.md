...
Available at no additional charge in all plans, including Free.
No DATEV registration, API subscription or transmission occurs.
...
Apply migration `037_datev_lodas_export.sql` before using the feature.
...
There is no paid-plan gate. A download records a generated file, not a successful
import into the payroll application.
