...
function client(failTable?: string) {
  ...
  const from = vi.fn((table: string) => {
    ...
    const chain = { select: vi.fn(), eq: vi.fn(), is: vi.fn(), lte: vi.fn(), gte: vi.fn(), or: vi.fn(), then: vi.fn() };
    for (const key of ["select", "eq", "is", "lte", "gte", "or"] as const) chain[key].mockReturnValue(chain);
